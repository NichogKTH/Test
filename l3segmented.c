...
int searchTypes(int searchMode, Medicine m[], int* pSizeOfM, char searchString[], int matches[]);                  //kan göra olika saker beroende på vem som kallar den
void registerMedicine(Medicine m[], int* pSizeOfM, char searchString[]);                            //många dependencies på andra funktioner i denna version


void unregisterMedicine(Medicine m[], int* pSizeOfM, int currentMedicine);                          //unregister medicine, flytta ner element som ligger efter denna
void addSize(Medicine m[], int currentMedicine);                                                    //find spot, flytta iväg element som ligger efter denna, insert storlek + saldo
void changeSaldo(Medicine m[], int currentMedicine);


void printMedicineTypes(int printMode, Medicine m[], int index);
...
void warnings(Medicine m[], int* pSizeOfM)
{
    ...
    for(i = 0; i < *pSizeOfM; i++)
    {
        ...
        printMedicineTypes(threshold, m, i);
    }
}
